...
import User from '../models/User.ts'; // Importa o modelo User (Assumindo que o caminho 'User.ts' está correto)
...
/**
 * Middleware para proteger rotas usando autenticação JWT (Bearer Token).
...
            // Use 'as string' pois process.env é sempre string ou undefined
            const secret = process.env.JWT_SECRET as string || 'sua_chave_secreta_padrao_muito_longa';
...
            // Assumindo que o ID está no payload como 'id' e que o seu modelo User tem 'findById'
            // Seleciona todos os campos, exceto a senha
            const user = await User.findById(decoded.id).select('-password');

            if (!user) {
                // Se o ID decodificado não corresponder a um utilizador
                return res.status(401).json({ message: 'Usuário não encontrado. Token inválido.' });
            }

            // 5. Anexa o objeto do usuário à requisição
            // O utilizador deve corresponder à interface IUser
